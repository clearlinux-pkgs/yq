#
# This file is auto-generated. DO NOT EDIT
# Generated by: autospec.py
#
Name     : yq
Version  : 3.0.1
Release  : 7
URL      : https://files.pythonhosted.org/packages/4b/31/6f26d1df7f8694479adeafa1f619dd2cfd9502089eac23bdf7fa342c9aaf/yq-3.0.1.tar.gz
Source0  : https://files.pythonhosted.org/packages/4b/31/6f26d1df7f8694479adeafa1f619dd2cfd9502089eac23bdf7fa342c9aaf/yq-3.0.1.tar.gz
Summary  : Command-line YAML/XML processor - jq wrapper for YAML/XML documents
Group    : Development/Tools
License  : Apache-2.0
Requires: yq-bin = %{version}-%{release}
Requires: yq-license = %{version}-%{release}
Requires: yq-python = %{version}-%{release}
Requires: yq-python3 = %{version}-%{release}
BuildRequires : buildreq-distutils3
BuildRequires : pypi(argcomplete)
BuildRequires : pypi(pyyaml)
BuildRequires : pypi(setuptools_scm)
BuildRequires : pypi(toml)
BuildRequires : pypi(xmltodict)

%description
yq: Command-line YAML/XML/TOML processor - jq wrapper for YAML, XML, TOML documents
===================================================================================

%package bin
Summary: bin components for the yq package.
Group: Binaries
Requires: yq-license = %{version}-%{release}

%description bin
bin components for the yq package.


%package license
Summary: license components for the yq package.
Group: Default

%description license
license components for the yq package.


%package python
Summary: python components for the yq package.
Group: Default
Requires: yq-python3 = %{version}-%{release}

%description python
python components for the yq package.


%package python3
Summary: python3 components for the yq package.
Group: Default
Requires: python3-core
Provides: pypi(yq)
Requires: pypi(argcomplete)
Requires: pypi(pyyaml)
Requires: pypi(toml)
Requires: pypi(xmltodict)

%description python3
python3 components for the yq package.


%prep
%setup -q -n yq-3.0.1
cd %{_builddir}/yq-3.0.1
pushd ..
cp -a yq-3.0.1 buildavx2
popd

%build
export http_proxy=http://127.0.0.1:9/
export https_proxy=http://127.0.0.1:9/
export no_proxy=localhost,127.0.0.1,0.0.0.0
export LANG=C.UTF-8
export SOURCE_DATE_EPOCH=1657041661
export GCC_IGNORE_WERROR=1
export AR=gcc-ar
export RANLIB=gcc-ranlib
export NM=gcc-nm
export CFLAGS="$CFLAGS -O3 -ffat-lto-objects -flto=auto "
export FCFLAGS="$FFLAGS -O3 -ffat-lto-objects -flto=auto "
export FFLAGS="$FFLAGS -O3 -ffat-lto-objects -flto=auto "
export CXXFLAGS="$CXXFLAGS -O3 -ffat-lto-objects -flto=auto "
export MAKEFLAGS=%{?_smp_mflags}
python3 setup.py build

pushd ../buildavx2/
export CFLAGS="$CFLAGS -m64 -march=x86-64-v3 -Wl,-z,x86-64-v3 "
export CXXFLAGS="$CXXFLAGS -m64 -march=x86-64-v3 -Wl,-z,x86-64-v3 "
export FFLAGS="$FFLAGS -m64 -march=x86-64-v3 -Wl,-z,x86-64-v3 "
export FCFLAGS="$FCFLAGS -m64 -march=x86-64-v3 "
export LDFLAGS="$LDFLAGS -m64 -march=x86-64-v3 "
python3 setup.py build

popd
%install
export MAKEFLAGS=%{?_smp_mflags}
rm -rf %{buildroot}
mkdir -p %{buildroot}/usr/share/package-licenses/yq
cp %{_builddir}/yq-3.0.1/LICENSE %{buildroot}/usr/share/package-licenses/yq/172ca3bbafe312a1cf09cfff26953db2f425c28e
python3 -tt setup.py build  install --root=%{buildroot}
echo ----[ mark ]----
cat %{buildroot}/usr/lib/python3*/site-packages/*/requires.txt || :
echo ----[ mark ]----
pushd ../buildavx2/
export CFLAGS="$CFLAGS -m64 -march=x86-64-v3 -Wl,-z,x86-64-v3 "
export CXXFLAGS="$CXXFLAGS -m64 -march=x86-64-v3 -Wl,-z,x86-64-v3 "
export FFLAGS="$FFLAGS -m64 -march=x86-64-v3 -Wl,-z,x86-64-v3 "
export FCFLAGS="$FCFLAGS -m64 -march=x86-64-v3 "
export LDFLAGS="$LDFLAGS -m64 -march=x86-64-v3 "
python3 -tt setup.py build install --root=%{buildroot}-v3
popd
/usr/bin/elf-move.py avx2 %{buildroot}-v3 %{buildroot} %{buildroot}/usr/share/clear/filemap/filemap-%{name}

%files
%defattr(-,root,root,-)

%files bin
%defattr(-,root,root,-)
/usr/bin/tomlq
/usr/bin/xq
/usr/bin/yq

%files license
%defattr(0644,root,root,0755)
/usr/share/package-licenses/yq/172ca3bbafe312a1cf09cfff26953db2f425c28e

%files python
%defattr(-,root,root,-)

%files python3
%defattr(-,root,root,-)
/usr/lib/python3*/*
